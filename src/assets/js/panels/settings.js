...
import { changePanel, accountSelect, database, Slider, config, setStatus, popup, appdata, setBackground, getErrorMessage, normalizeError } from '../utils.js'
        ...
        document.querySelector('.accounts-list').addEventListener('click', async e => {
            ...
            try {
                ...
                const normalizedError = normalizeError(err, {
                    code: 'SETTINGS_ACCOUNT_ERROR',
                    message: 'Impossible de mettre a jour le compte selectionne.'
                });
                console.error('Settings account action failed', normalizedError)
                popupAccount.openPopup({
                    title: 'Erreur',
                    content: getErrorMessage(normalizedError, 'Une erreur est survenue lors de la gestion du compte.'),
                    color: 'red',
                    options: true
                })
            } finally {
                if (popupAccount.popupOptions.style.display === 'none') popupAccount.closePopup();
            }
        })
    }
